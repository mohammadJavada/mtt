...
export default function LikeIconInCards({
  ...
  useEffect(() => {
    if (
      ...
      httpService.get(`${FRONT2DB}/AdSale/Get/Likes`).then((res) => {
        return res.data.ads?.map((item: { advertiser_id: number | string }) => {
      ...
      httpService.get(`${AUTH_URL}/Auth/Anonymous`).then((res) => {
        res.data?.user?.Likes?.map((item: number | string) => {
    ...
    httpService
      ...
      .then((res) => {
        ...
        if (res.status == 200 && userInfo.phone_number) {
          toast.info("آگهی به لیست پسندیده شده‌ها اضافه شد.  ");
        }
        if (position == index) {
          setIsliked(true);
      ...
      .catch(() => {
        setLoading(false);
        toast.error("آگهی به لیست پسندیده شده‌ها اضافه نشد.");
      });
  };
  ...
  const handleDisLike = (e: React.MouseEvent, position: number) => {
    ...
    httpService
      ...
      .catch(() => {
        setLoading(false);
        toast.error("آگهی از لیست پسندیده شده ها حذف نشد.");
      });
  };
